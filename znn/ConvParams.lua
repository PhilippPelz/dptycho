...
function c:updateOutput(input)
  assert(self.tmp:nElement() == self.weight:nElement(), 'temporary storage has to be same size as filters')
...
function c:accGradParameters(input, gradOutput)
    ...
    -- pprint(self.gradWeight)
    -- pprint(self.tmp)
    -- plt:plot(gradOutput[1]:float(),'ConvParams gradOutput')
    self.tmp:zero()
    ...
    -- plt:plotcompare({self.gradWeight[1][1]:float(),self.gradWeight[2][1]:float()})
    -- pprint(self.gradWeight)
    -- pprint(self.gradWeight)
