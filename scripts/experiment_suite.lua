...
function main()
  ...
  local overlap = {0.5}
  local nu = {9e-1,6e-1,3e-1,1e-1,5e-2}
  ...
  par.object_init_truncation_threshold = 96
  ...
  for probe_type = 3,3 do
    ...
    elseif probe_type == 3 then
      ...
      local pi = f:read('/pi'):all()
      probe = torch.ZCudaTensor(pr:size()):copyRe(pr:cuda()):copyIm(pi:cuda())
      -- plt:plot(probe:zfloat(),'probe_blr')
      f:close()
    end
    local ID = 10
    for i,dose0 in ipairs(dose) do
      for j,overlap0 in ipairs(overlap) do
        ...
        for k,nu0 in ipairs(nu) do
          ...
          -- for i=1,par.a:size(1) do
          --   plt:plot(par.a[i]:float(),'a')
          -- end
          -- print()
          -- print('a sum')
          -- print(par.a:sum())
