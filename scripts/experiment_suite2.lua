...
function get_data(pot_path,dose,overlap,N,E,probe,probe_diameter)
  ...
  local probe_mask = torch.ge(probe_int:re(),1e-1):int()
...
function main()
 ...
 local electrons_per_angstrom = {5.62341325,    10.        ,    17.7827941 ,    31.6227766 ,
  ...
  local electrons_per_angstrom = {5.62341325,    10.        ,    17.7827941 ,    31.6227766 ,
          56.23413252,   100.        ,   177.827941  ,   316.22776602,
         562.34132519}
  local overlap = {0.81}--,0.7,0.75,0.8}0.45,0.5,0.55,0.6,
  -- local overlap = {0.45}
  local nu = {10e-2}--{10e-2,5e-2,1e-2,5e-3}--4e-2,2e-1,1e-1,
  local lr = {1e-5}--{1e-4,5e-4}
  ...
  par.optimizer = optim.sgd -- nag sgd cg

  par.regularization_params = {}
  par.regularization_params.amplitude = 2e-1
  ...
  par.experiment.z = 2.2
  par.experiment.E = E
  par.experiment.det_pix = 80e-6
  ...
  for probe_type = 0,5 do
    ...
    -- probe = s:focused_probe(E, N, 0.85, 9.8e-3, 4e2, 0 , 0, 0 ,0 , 5 , false)
    ...
    -- elseif probe_type == 3 then
      -- probe = s:random_probe2(N,(0.10 + probe_type * 0.02),0.22,0.00)
      local f = hdf5.open('/home/philipp/drop/Public/probe_fzp7.h5','r')
      local pr = f:read('/pr'):all()
      local pi = f:read('/pi'):all()
      probe = torch.ZCudaTensor(pr:size()):copyRe(pr:cuda()):copyIm(pi:cuda())
      -- plt:plot(probe:zfloat(),'probe')
      -- probe_diameter = 20
      probe_diameter = 0.2*N
      -- f:close()
